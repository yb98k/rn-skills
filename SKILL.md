...
description: >
  ...
  自动完成素材收集、选题筛选、文案生成、AI配图（OpenAI GPT Image）、本地归档的完整流程。
...
3. 调用 OpenAI GPT Image API（`$OPENAI_API_KEY`）：
   ```bash
   curl -s "https://api.openai.com/v1/images/generations" \
     -H "Content-Type: application/json" \
     -H "Authorization: Bearer $OPENAI_API_KEY" \
     -d '{"model":"gpt-image-1","prompt":"{完整prompt}","n":1,"size":"1024x1536","quality":"high"}'
   ```
4. 将返回的 base64 数据或 URL 保存为 PNG 文件
5. 6张图片可并行生成。API 失败则跳过图片，文案中标注「图片待补充」
...
- 穿搭建议要具体可操作，标注单品类型
- 文案结尾必须有互动引导
- 每次生成控制在12张图以内（2笔记 x 6张），预计API成本约 $0.24-0.48
- 环境变量 `OPENAI_API_KEY` 需提前设置
